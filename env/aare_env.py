...
class AAREEnv(Environment):
    ...
    def reset(self, seed=None, episode_id=None, **kwargs):
        ...
        self.attack_engine.reset_chain()
        self.step_count = 0
        self.attack_sequence = TASK_ATTACKS.get(task, TASK_ATTACKS["medium"])
        self.sequence_index = 0
        self.max_steps = len(self.attack_sequence)
        self.current_attack = self.attack_sequence[self.sequence_index]
    ...
    def step(self, action, timeout_s=None, **kwargs):
        ...
        # Store for inference.py compatibility
        self._last_reward = reward
        self._last_done = done
        self._last_info = {"attack_blocked": attack_blocked, "risk_score": self.app_state.risk_score}

        # MUST return only the observation — reward and done go inside it
    ...
    def _get_observation(self, reward=None, done=False):
        return AAREObservation(
            attack_type=self.current_attack,
            target_component="web_app",
            severity=ATTACK_SEVERITY[self.current_attack],
